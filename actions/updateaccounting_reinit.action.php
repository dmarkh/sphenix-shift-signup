...
function updateaccounting_reinit_action() {
	...
	$inst[$iid]['nexperts'] += $inst_expertcredits[$k];
        //$inst[$iid]['nemeritus'] = ( $inst[$iid]['nemeritus'] ? $inst[$iid]['nemeritus'] + intval($inst_stat[$k]['emeritus']) : intval($inst_stat[$k]['emeritus']) );
        $inst[$iid]['nemeritus'] += ( $inst_stat[$k]['emeritus'] ? intval($inst_stat[$k]['emeritus']) : 0 );
    } 
/*
// get list of the institutions:
...
// get emeritus
    $emeritus = $db->Query('SELECT InstitutionId AS id, COUNT(*) AS nemeritus FROM `starweb`.members WHERE isEmeritus = "Y" AND (LeaveDate > NOW() || LeaveDate = "0000-00-00") AND InstitutionId IN ('.$inst_keys.')  AND (DisabledDate = "0000-00-00 00:00:00" OR DATEDIFF(NOW(), DisabledDate) < 14) GROUP BY InstitutionId');
    foreach ($emeritus as $k => $v) {
	$inst[$v['id']]['nemeritus'] = $v['nemeritus'];
    }
*/
    foreach($inst as $k => $v) {
	//$eff = intval($v['nauth']) - intval($v['nexperts']) - intval($v['nemeritus']);
	$eff = intval($v['nauth']) - intval($v['nemeritus']); // just remove emeritus..
	if ($eff < 0) { $eff = 0; }
	$sql = 'UPDATE `ShiftSignup`.ShiftAdmin SET effective_authors = '.$eff.' WHERE institution_id = '.intval($k);
	//echo $sql."\n";
	$res = $db->Query($sql);
    }
...
/*
	...
	$experts = array();
	$exp = $db->Query('SELECT InstitutionId AS id, COUNT(*) AS nexperts FROM `starweb`.members WHERE isExpert = "Y" AND (LeaveDate > NOW() || LeaveDate = "0000-00-00") AND InstitutionId IN ('.$inst_keys.') GROUP BY InstitutionId');
	foreach ($exp as $k => $v) {
	  # echo '->'.$v['id'].' ';
	...
	if ($scale != 1.0) {
        ...
        foreach ($shifts as $k => $v) {
            $res = $db->Query('REPLACE INTO `ShiftSignup`.ShiftAdmin (institution_id, detector_experts, shifts_required, effective_authors, historical_data) VALUES ('.$k.', 0, '.intval($shifts[$k]).','.intval($authors[$k]).',"'.$historical_data[$k].'")');
        }
    }
*/
/*
    ...
    $authors = $db->Query('SELECT InstitutionId AS id, COUNT(*) AS nauth FROM `starweb`.members WHERE isAuthor = "Y" AND LeaveDate > NOW() AND InstitutionId IN ('.$inst_keys.')  AND (DisabledDate = "0000-00-00 00:00:00" OR DATEDIFF(NOW(), DisabledDate) < 14) GROUP BY InstitutionId');
    foreach ($authors as $k => $v) {
	$inst[$v['id']]['nauth'] = $v['nauth'];
    }

    $experts = $db->Query('SELECT InstitutionId AS id, COUNT(*) AS nexperts FROM `starweb`.members WHERE isExpert = "Y" AND LeaveDate > NOW() AND InstitutionId IN ('.$inst_keys.')  AND (DisabledDate = "0000-00-00 00:00:00" OR DATEDIFF(NOW(), DisabledDate) < 14) GROUP BY InstitutionId');
    foreach ($experts as $k => $v) {
	$inst[$v['id']]['nexperts'] = $v['nexperts'];
    }
*/
    $stats = $db->Query('SELECT institution_id AS id, detector_experts, shifts_required, effective_authors, historical_data FROM `ShiftSignup`.ShiftAdmin WHERE institution_id IN ('.$inst_keys.')');    
