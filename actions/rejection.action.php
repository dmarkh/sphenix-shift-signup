...
function rejection_action() {
    ...
    $in = get_inst_institutions();

    $inst = array();
    foreach($in as $k => $v) {
	if ( empty($v['name']) || empty($v['id']) ) { continue; }
	...
	if (empty($inst[$v['institution_id']]) ) { continue; } 
    ...
    //$m = $db->Query('SELECT Id, InstitutionId, FirstName, LastName, isAuthor, isExpert, Expertise, isEmeritus, UNIX_TIMESTAMP(NOW()) as tsnow, UNIX_TIMESTAMP(LeaveDate) as tsld, (UNIX_TIMESTAMP(NOW()) - UNIX_TIMESTAMP(LeaveDate)) as tsdiff FROM `starweb`.members WHERE InstitutionId IN ('.$inst_keys.') ORDER BY LastName ASC');
    $m = get_inst_members();    
    
    foreach($m as $k => $v) {
	if ( empty($inst[$v['InstitutionId']]) ) { continue; }
	...
	//----- uncomment to use first historical record as "current" data -----
	...
	if ( empty( $inst[$k]['active_authors'] ) ) {
	    unset($inst[$k]);
	    continue;
	}

	if ($v['shifts_required'] > 0) {
	  $r = round($v['shifts_taken'] / $v['shifts_required'], 4);
	  if ($r > 1.0) { $r = 1.0; }
	  $inst[$k]['r'] = $r;
	} else {
	  $inst[$k]['r'] = 1.0;
	...
	// missed percentage
	$inst[$k]['pct_miss'] = (1.0 - floatval($inst[$k]['r'])) * 100.0; 
	//echo $inst[$k]['pct_miss'].' ::: ';
	// D raw
    ...
    foreach ($inst as $k => $v) {
	$inst[$k]['sort'] = 0;    
	if (floatval($v['pct_miss']) < floatval($par['M'])) {
	   // do nothing, not guilty this year..
		...
		$ctr += 1;
		// ">=" for concidering "current" status, just ">" for historical values only
		if ($ctr > $par['P']) break; // past X years scan only, or whatever history inst has..
	    }

	    # JL - we now calculate as an average and if > M, bang!
...
//    print_r($rej);
    ...
    $tpl->set('warn', $warn);
    $tpl->set_file('rejection.tpl.php');
}
...
function get_historical_data($source) {
    ...
    //FIXME: starweb
    $tks = $db->Query('SELECT Id AS id, InstitutionName AS name FROM `starweb`.institutions WHERE ISNULL(LeaveDate) = true OR LeaveDate > NOW() OR LeaveDate = 0 ORDER BY InstitutionName ASC');
    foreach($tks as $k => $v) {
	$inst[$v['id']]['name'] = $v['name'];
    }
