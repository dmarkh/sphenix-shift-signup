<?php
	...
	*/
	eval("\$session_var=".$first_num.$second_num.$third_num.";");
	/* 
		save the operation result in session to make verifications
	*/
	$_SESSION['security_number'] = md5(sha1($session_var.$session_id, false),false);
	/*
	...
	*/
	...
	imagettftext(
		...
		rand( -$angle , $angle ),
		rand( 10, $item_space-32 ),
		rand( 32, $captcha_h - 15 ),
		$black,
		$font_path,
		$first_num);
	...
	imagettftext(
		...
		rand( -$angle, $angle ),
		rand( $captcha_w/2.-15, $captcha_w/2 ),
		rand( $captcha_h/2., $captcha_h/2.+20 ),
		$black,
		$font_path,
		$second_num);
	...
	imagettftext(
		...
		rand( -$angle, $angle ),
		rand( 2*$item_space, 3*$item_space-32),
		rand( 32, $captcha_h-15 ),
		$black,
		$font_path,
		$third_num);
